...
declare_id!("8DRM9cCGhViBjcvkSdzxccS2wsQ5FGwMJFiBeftQLiNL");
...
mod dfk_points {
    ...
    pub fn conn_discord(ctx: Context<Vote>) -> ProgramResult {
        ctx.accounts.vote_account.counter += 200;
        Ok(())
    }

    pub fn follow_twitter(ctx: Context<Vote>) -> ProgramResult {
        ctx.accounts.vote_account.counter += 350;
        Ok(())
    }

    pub fn retweet_tweet(ctx: Context<Vote>) -> ProgramResult {
        ctx.accounts.vote_account.counter += 250;
        Ok(())
    }

    pub fn turnon_notif(ctx: Context<Vote>) -> ProgramResult {
        ctx.accounts.vote_account.counter += 500;
        Ok(())
    }
}
